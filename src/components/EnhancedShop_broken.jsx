...
const EnhancedShop = () => {
  ...
  // Load products from inventory (published only)
  useEffect(() => {
    const loadProducts = () => {
      // Load from inventory module
      const inventoryProducts = JSON.parse(localStorage.getItem('dankdash_inventory_products') || '[]');
      
      // Filter only published products for customer view
      const publishedProducts = inventoryProducts.filter(product => product.published === true && product.currentStock > 0);
      
      // Transform inventory format to shop format
      const shopProducts = publishedProducts.map(product => ({
        id: product.id,
        name: product.name,
        ...
        strain: product.category === 'Flower' ? 'Hybrid' : 'N/A',
        effects: ['Premium Quality'],
        flavors: ['Natural'],
        stock: parseInt(product.currentStock),
        status: product.status === 'In Stock' ? 'active' : 'inactive',
        sku: product.sku,
      ...
      // Fallback to mock data if no inventory products
    ...
    const interval = setInterval(loadProducts, 5000); // Check every 5 seconds
